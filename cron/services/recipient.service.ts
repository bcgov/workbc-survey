  ...
  try {
    await db
      ...
      .then((resp: any) => {
        recipients = {
          count: resp.rowCount,
          recipients: resp.rows.map((c: any) => {
            return {
              id: c.id,
...
export const updateRecipientReminder = async (reminder: any, id: string) => {
  ...
  try {
    await db
      .query(
        format(
          `UPDATE api.recipients SET "reminders" = coalesce(reminders::jsonb, '{}'::jsonb) || '%s'::jsonb WHERE id = %L`,
          reminder,
          id
        )
      )
...
export const getReminderRecipients = async (
  interval: number,
  surveyType: string,
  reminder: string
) => {
  let recipients: any;
  let query = format(
    `SELECT * FROM api.recipients WHERE "surveyCompleted" IS FALSE AND "surveyDate" < (NOW() - '%s day'::interval) AND "surveyType" = %L AND "emailError" IS FALSE AND NOT coalesce(reminders::jsonb, '{}'::jsonb) ? %L`,
    interval,
    surveyType,
    reminder
  );
